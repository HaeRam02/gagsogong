...
function TaskCreateView() {
  ...
  const handleSubmit = async (e) => {
    ...
    const selectedEmployee = employees.find(
      (emp) => emp.employeeId === formData.assignee
    );
    ...
    const taskDtoPayload = {
      ...
      publicStartDate:
        formData.visibility === "public" ? formData.publicStartDate : null,
      publicEndDate:
        formData.visibility === "public" ? formData.publicEndDate : null,
    };

    uploadData.append(
      "taskDto",
      new Blob([JSON.stringify(taskDtoPayload)], { type: "application/json" })
    );
