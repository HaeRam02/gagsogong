...
public interface ParticipantRepository extends JpaRepository<Participant, Long> {
    ...
    @Query("SELECT COUNT(p) > 0 FROM Participant p WHERE p.participantId = :id")
    ...
    @Query("""
    SELECT p.employeeId, e.name, d.deptTitle
    FROM Participant p
    JOIN Employee e ON p.employeeId = e.employeeId
    JOIN Department d ON e.department.deptId = d.deptId
    WHERE p.scheduleId = :scheduleId
    ORDER BY e.name
    """)
